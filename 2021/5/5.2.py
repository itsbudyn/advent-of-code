...
def diagdraw(matrix,p1,p2):
    if p1[0]<p2[0]: xs_diag=list(range(p1[0],p2[0]+1))
    else: xs_diag=list(range(p2[0],p1[0]+1))[::-1]

    if p1[1]<p2[1]: ys_diag=list(range(p1[1],p2[1]+1))
    else: ys_diag=list(range(p2[1],p1[1]+1))[::-1]

    print(xs_diag)
    print(ys_diag)
    print("\n\n")

    for i in range(len(xs_diag)):
        matrix[ys_diag[i]][xs_diag[i]]+=1
